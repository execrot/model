  ...
  private function _parse(Model $model)
  {
    ...
    foreach ($docblock as $line) {
      ...
      if (substr($line, 0, strlen(self::ID_COLLECTION)) == self::ID_COLLECTION) {
        $this->_collection = ucfirst(strtolower(trim(str_replace(self::ID_COLLECTION, '', $line))));
        continue;
      }

      if (substr($line, 0, strlen(self::ID_PRIMARY)) == self::ID_PRIMARY) {
        $this->_primary = trim(str_replace(self::ID_PRIMARY, '', $line));
        continue;
      }
      ...
      if (substr($line, 0, strlen(self::ID_PROPERTY)) == self::ID_PROPERTY) {
        ...
        $property->setName(str_replace('$', '', $propertyLine[2]));
